...
import { RefreshCw, Shield, AlertTriangle, Activity, Users, Globe, ArrowLeft, Key, TestTube } from "lucide-react";
...
interface HealthStatus {
    ...
    environment: string;
    checks: {
        database: { status: 'pass' | 'warn' | 'fail'; duration: number; message?: string };
        environment: { status: 'pass' | 'warn' | 'fail'; duration: number; message?: string };
        security: { status: 'pass' | 'warn' | 'fail'; duration: number; message?: string };
        services: { status: 'pass' | 'warn' | 'fail'; duration: number; message?: string };
    };
}
...
export default function SecurityDashboard() {
    ...
    const clearLogs = async () => {
        ...
        try {
            const response = await fetch('/api/security/monitor', { method: 'DELETE' });
            if (response.ok) {
    ...
    return (
        <div className="container mx-auto p-6 space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground">
                        Real-time security monitoring, system health, and administrative tools
                    </p>
                </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            {Object.entries(healthStatus.checks).map(([check, data]) => (
                                <div key={check} className="text-center">
                                    <div className={`text-lg font-semibold ${getHealthColor(data.status)}`}>
                ...
                <>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <Card>
                            ...
                            <CardContent>
                                <div className="text-3xl font-bold">{securityStats.totalEvents}</div>
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="text-3xl font-bold">{securityStats.last24Hours.total}</div>
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="text-3xl font-bold">{securityStats.suspiciousIPs.length}</div>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                {Object.entries(securityStats.last24Hours.byType).map(([type, count]) => (
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                {Object.entries(securityStats.last24Hours.bySeverity).map(([severity, count]) => (
                                    <div key={severity} className="flex justify-between items-center p-3 border rounded">
                ...
                <CardContent>
                    <div className="flex flex-wrap gap-4">
                        <Button asChild size="sm">
                            <Link href="/api/csrf-token" target="_blank" className="flex items-center gap-2">
            ...
            <div className="text-center text-sm text-muted-foreground">
                Logged in as: {session?.user?.email} | Last updated: {new Date().toLocaleString()}
            </div>
        </div>
    );
}