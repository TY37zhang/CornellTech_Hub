...
export default function EditCreditTransferModal({
  ...
  const getAvailableCreditsForEdit = (): number => {
    ...
    const otherTransfersFromCategory = existingTransfers
      .filter(
        (t) => t.id !== transfer.id && t.fromCategory === transfer.fromCategory,
      )
      .reduce((sum, t) => sum + t.amount, 0);

    const actualNetCredits =
      creditInfo.totalCredits -
      creditInfo.ethicsDeduction +
      creditInfo.ethicsAddition -
      otherTransfersFromCategory;
  ...
  const previewTransferImpact = () => {
    ...
    const fromCreditsAfter =
      fromCredits.netCredits + transfer.amount - transferAmount;
    const toCreditsAfter =
      ...
      <div className="mt-4 p-4 bg-muted/50 rounded-none">
        ...
        <div className="space-y-2 text-sm">
          ...
          {oldToCredits && toCategory !== transfer.toCategory && (
            <div>
              <span className="font-medium">
                {getRequirementDisplayName(transfer.toCategory)} (old):
              </span>{" "}
              {oldToCredits.netCredits} cr →{" "}
              {oldToCredits.netCredits - transfer.amount} cr
            </div>
          )}
          <div>
            <span className="font-medium">
            ...
            {toCategory === transfer.toCategory
              ? fromCredits.netCredits
              : toCredits.netCredits}{" "}
            cr → {toCreditsAfter} cr
          </div>
        </div>
      </div>
    );
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Modify this credit transfer. Changes will update your credit
