      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4">
          {selectedCourses.map((course) => (
            <div key={course.id} className="p-4 border rounded-none">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                {Object.entries(requirements).map(([key]) => {
                  const isAssigned = findCourseAssignment(course.id) === key;
                  return (
                    <Button
                      key={key}
                      ...
                      size="sm"
                      className="w-full"
                      onClick={() => handleRequirementClick(course, key)}
                    >
                      {isAssigned ? "Remove from" : "Add to"}{" "}
          ...
          {selectedCourses.length === 0 && (
            <div className="text-center text-muted-foreground py-8">
              Select courses from the search above to assign them to
              requirements
            </div>
          )}
        </div>
